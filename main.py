...
class PongGame:
    ...
    def run_game(self):
        while True:
            ...
            if self.game_active:
                ...
                score_point = self.ball.update(self.r_paddle, self.l_paddle)
                self._score(score_point)
    ...
    def _draw_table(self):
        ...
        pygame.draw.rect(self.screen, self.settings.color_white,
                         (self.settings.screen_width - self.r_paddle.width, 0, self.settings.screen_width,
    ...
    def _score(self, score_point):
        if score_point is False:
            self.p_points += 1
            self.ball.reset_ball()
        elif score_point is True:
            self.c_points += 1
            self.ball.reset_ball()
    ...
    def _win_lose_condition(self):
        if self.p_points >= 9:
            ...
            if self.sound:
                pygame.mixer.Sound.play(self.settings.win_sound)
                self.sound = False
        ...
        if self.c_points >= 9:
            ...
            self.menu_state = None
            if self.sound:
                pygame.mixer.Sound.play(self.settings.lose_sound)
                self.sound = False
    ...
    def menu(self):
        if self.game_active is False and self.menu_state == 'main':
        ...
        if self.game_active is False and self.menu_state == 'options':
        ...
        if self.game_active is False and self.menu_state == 'settings':
